...
.. NOTE::
  ...
  Tuleap 16.6 is currently under development.

Configuration change for email relay using ``STARTTLS``
-------------------------------------------------------

If your email relay is using ``STARTTLS`` you must reset the configuration variable ``email_relayhost_smtp_use_tls`` to the default
value with:

.. sourcecode:: shell

    tuleap config-reset email_relayhost_smtp_use_tls
