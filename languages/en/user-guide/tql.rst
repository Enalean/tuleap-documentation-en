...
When you use a semantic, at least one of selected trackers must have it configured and field linked to the semantic must be readable by the current user.
...
If **none** of the trackers does not define the "Status" semantic, it will cause an error to be shown. The same is true for permissions: if **none** of the "Status" fields is not readable by the current user, an error will be displayed.

When you use a dynamic field, for at least one of selected trackers it must be added in the tracker and it must be readable by the current user.
...
For example, if you run an expert query containing ``@last_update_date``, at least one of selected trackers **must** have a "Last update date" field and that field **must** be readable by the user viewing the widget.
If **none** of the trackers does not have a "Last update date" field, it will cause an error to be shown. The same is true for permissions: if **none** of the "Last update date" fields is not readable by the current user, an error will be displayed.
...
.. attention::
  ...
  You can follow advancement in `epic #37567 SuperTableau - Full TQL mode <https://tuleap.net/plugins/tracker/?aid=37567>`
