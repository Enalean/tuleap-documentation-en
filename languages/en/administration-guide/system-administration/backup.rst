...
This documentation is here to help you set your backup up. Be careful with this, it's just a guide and you will probably want to backup more things
...
First you need to identify the old ids from your backup archive, looking for each of those files (owned by) :
...
Exemple use of ``ls -ldn`` to display those ids
...
As you can see, for ftpadmin uids and gids are not the same in this exemple.

Finally you can use a tool like ``rsync`` to help you remap your data while restoring/resyncing:

``rsync --usermap=980:codendiadm,976:gitolite,979:ftpadmin --groupmap=980:codendiadm,976:gitolite,978:ftpadmin [OPTION...] SRC... [DEST]``


External disk
`````````````

For the mentioned directories tuleap is using, you could move them on a **separate data disk** for easier backup.
...
Then, after suspending **all services including mysql** (if you want mysql to be on a data disk too) :
  ...
  - then at your convenience:
    * create symbolic links for each directory exemple: ``ln -s /data/etc_nginx /etc/nginx``
    * or use bind mounts thru ``/etc/fstab`` like below (you need to ``mount`` them after updating the file)
  - you can then restart services

Exemple of bind mounts via local fstab
.. code-block:: bash
