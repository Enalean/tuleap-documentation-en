...
* Never create unneeded computed properties. If a property only returns ``$gettext()``, use ``v-bind`` instead: ``v-bind:title="$gettext('No color')"`` is better than ``v-bind:title="my_custom_title"``. It will also help for test coverage.
